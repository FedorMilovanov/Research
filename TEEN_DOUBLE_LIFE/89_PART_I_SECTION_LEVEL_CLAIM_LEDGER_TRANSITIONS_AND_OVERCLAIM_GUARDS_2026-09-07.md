# Part I — Section-Level Claim Ledger / Transitions / Overclaim Guards

**Status:** PRE-DRAFT EDITORIAL CONTROL / RESEARCH ONLY / NOT PUBLICATION  
**Date:** 2026-09-07  
**Depends on:** modules `80`, `85`, `87`, `88`; 2026-09-08 deltas in `18`, `27`, `29`, `64`.  
**Goal:** freeze what each section is actually allowed to prove before any polished prose is written.

---

# 0. Editorial rule

Every section must have:

1. **one governing question**;
2. **one primary claim**;
3. **one biblical center**;
4. **limited supporting evidence**;
5. **one explicit overclaim guard**;
6. **one transition that advances the argument rather than repeating it**.

If a paragraph cannot be assigned to a section-level claim, it is likely research residue rather than publication material.

The 2026 AI-companion and coercive-sexting deltas **do not create new numbered main sections**. They attach only where they sharpen an already-owned claim.

---

# 1. Opening — “You may know only one version of the child”

## Governing question

What exactly do we mean by a “double life,” and why should a believing family take it seriously without assuming every church-raised teenager is secretly depraved in the same visible ways?

## Primary claim

> A double life becomes morally serious when a child deliberately maintains a respectable home/church presentation while protecting a contradictory hidden pattern of sin, deception, and peer allegiance.

## Evidence

- biblical concept of hypocrisy/hidden conduct can be referenced later;
- no statistic needed;
- use a clearly labeled **composite pattern**, not an invented “true story.”

## Required nuance

- normal privacy ≠ double life;
- adolescent immaturity ≠ settled hypocrisy;
- one fall ≠ second identity;
- not all ДВР do this.

## Forbidden

- “Every Christian parent has a second child hidden in the phone.”
- voyeuristic examples.

## Transition

> If the hidden life is real, the first question is not why the phone is so powerful. It is why the forbidden life tastes good enough to protect.

---

# 2. Section — “Украденные воды сладки”

## Governing question

Why is the easier sinful fruit often more attractive than restraint, humility, truth, and the narrow way?

## Primary claim

> Scripture itself recognizes that sin can offer real immediate pleasure while concealing its end.

## Biblical center

- Prov. 9:17–18;
- Heb. 11:25;
- James 1:14–15.

## Classical support

Brooks:

- bait/hook;
- minimization;
- promise of easy later repentance.

## Required distinction

The attraction is not an excuse.

The sinner is not merely fooled by an external marketer; his own desire responds to the bait.

## Forbidden

- “Sin is never pleasurable.”
- “Pleasure proves addiction.”
- reducing narrow-way discipleship to generic delayed gratification.

## Transition

> But desire does not grow in a vacuum. A teenager also learns what his peers laugh at, admire, share, and reward.

---

# 3. Section — “Компания, которая больше не уходит домой”

## Governing question

What has changed when the peer group remains continuously available after school, church, and physical separation?

## Primary claim

> Companionship has always shaped conduct; digital communication can make the peer world unusually persistent and private.

## Biblical center

- 1 Cor. 15:33;
- 2 Tim. 2:22 may be held mostly for Part II/III but can supply positive contrast.

## Modern anchor

WHO/HBSC:

- almost 280,000 respondents;
- ages 11/13/15;
- 44 countries/regions;
- 36% reported continuous online contact with friends.

## Concrete examples

- group chat after parents think the social day ended;
- memes/profanity/sexual humor;
- private DMs;
- status/approval.

## Required nuance

Peer influence does not erase personal responsibility.

Holy peers can form positively too.

## Forbidden

- “Peers replace parents at puberty.”
- “36% are social-media addicts.”

## Transition

> Persistent company becomes much more powerful when it also carries a private doorway to content, conversations, and people that once required physical access.

---

# 4. Section — “Карманная инфраструктура тайной жизни”

## Governing question

What precisely has the smartphone/internet changed without changing the human heart?

## Primary claim

> The digital age drastically lowers friction around access, privacy, repetition, peer presence, recommendation, concealment—and now can also provide persistent private conversation even when no human peer is online.

## Modern anchors

Core:

Pew 2024, U.S. teens 13–17:
- 96% daily internet use;
- nearly half online almost constantly;
- 95% smartphone access.

Children’s Commissioner:
- pornography may be encountered on mainstream platforms;
- accidental exposure is documented;
- age-of-first-exposure numbers must carry sample/geography guard.

Optional 2026 delta if the paragraph needs one sentence on conversational AI:

Pew/eSafety:
- a measurable minority of teens use chatbots for casual conversation/emotional support;
- official child-safety research documents companion-like use and some harmful/sexualized interactions.

Do **not** automatically add another percentage if the structural sentence works without it.

## Structural list

Keep short in prose:

`access / privacy / persistence / repetition / recommendation / concealment`.

Possible additional concept:

`peerless hidden companionship` — private relationship-like AI conversation without a real peer online.

This is editorial synthesis, not a diagnostic or prevalence category.

## Required theological sentence

> New opportunity is not new depravity.

AI does not become a fourth spiritual enemy beside world/flesh/devil.

## Forbidden

- `internet ×20`;
- “algorithm causes lust”;
- “phone is the fourth enemy beside world/flesh/devil”;
- “most teenagers now have AI girlfriends/boyfriends”;
- “talking to AI is itself sin.”

## Transition

> No modern pathway shows the marriage of old desire and lowered friction more clearly than pornography: old lust, unprecedented logistics.

---

# 5. Section — “Порнография без мифологии о мозге”

## Governing question

How can the article speak with biblical severity about pornography without borrowing questionable addiction/neuroscience rhetoric or teaching the reader to hear “sex/body” as synonymous with “dirty”?

## Primary claim

> Pornography is morally serious sexual impurity/lust regardless of whether a particular teenager meets any clinical category; digital access can make exposure, repetition, secrecy, and sexual scripting much easier.

## Biblical center

- Matt. 5:27–30 may be introduced briefly but developed in Part II;
- 1 Cor. 6 / 1 Thess. 4 may be candidate texts if final exegesis packet chooses them;
- Prov. 5 supplies the positive canonical counterweight: sexual desire and delight are not treated as dirty in themselves but ordered covenantally within marriage.

## Required positive sexual-ethics guard

Part I must contain at least one clear reader-facing sentence establishing the positive side:

> **Biblical sexual ethics is not hostility to the body, attraction, desire, or marital intimacy. The same wisdom tradition that warns fiercely against adulterous seduction also celebrates sexual delight within marriage; the evil is not that God made sexuality good, but that sinners detach the gift from God’s order and turn another person into an object of lust, status, secrecy, exploitation, or self-gratification.**

This is synthesis, not a direct quotation. Keep it concise enough that Part I does not become a marriage-theology excursus.

Why this is P0 rather than decorative balance:

- without it, a long pornography/sexual-danger section can accidentally catechize the reader into `sexuality = danger/dirt`;
- shame-driven families may already lack a healthy language for attraction/body/desire;
- the project’s anti-pornography argument is stronger, not weaker, when it distinguishes corruption of a good gift from condemnation of the gift itself.

## Modern evidence

Use only:

- strongest systematic/longitudinal review already in Research;
- Children’s Commissioner exposure/pathway facts.

## Required clinical guard

`porn use` ≠ `PPU` ≠ `CSBD`.

Moral guilt does not require a clinical disorder.

## Forbidden

- universal escalation;
- irreversible brain damage;
- fixed exposure count = addiction;
- shame = proof of disorder;
- “sexual desire itself is dirty”;
- “a godly teenager should have no sexual attraction”;
- using pornography’s corruption as proof that embodiment or marital sexuality is suspect.

## Transition

> The deeper problem is not only what the teenager sees. It is what he must begin doing with truth in order to keep seeing it.

---

# 6. Section — “Чтобы сохранить грех, приходится строить тайну”

## Governing question

Why does repeated hidden sin so often produce deception beyond the original act?

## Primary claim

> Protected sin tends to require protected access: concealment, partial truths, alternate accounts, edited stories, and management of what parents/church can know.

## Biblical center candidates

- Prov. 28:13 may be saved for Part II repentance;
- John 3 light/darkness may be considered;
- Ps. 32 can be considered if not overloaded.

## Research support

- secret-account material;
- disclosure/secrecy evidence stays mostly backstage.

## Required nuance

Do not treat:

- every password;
- every private conversation;
- every second account

as proof of sexual sin.

## Main image

The teenager increasingly manages **information flow** between worlds.

## Forbidden

- “If you do not know every password, your child is hiding sin.”

## Transition

> The longer truth must be managed around a desire, the easier it becomes for the desire to receive practical priority over the truth the child still knows.

---

# 7. Section — “Он всё ещё знает — но почти уже не спорит”

## Governing question

How can someone retain correct Christian answers while repeatedly acting as though those answers have no authority in the moment?

## Primary claim

> Biblical hardening need not begin with a conscious new creed; sin can deceive, dull sensitivity, entangle desire, and obtain repeated yielding while moral knowledge remains.

## Biblical center

- Heb. 3:12–13;
- Eph. 4:17–19;
- Titus 3:3;
- Rom. 1:32 sparingly.

## Classical center

Owen, *Indwelling Sin*:

`mind drawn aside → affections entangled → will consents`.

Owen on convictions fading if truth does not gain deeper practical rule.

## Key wording

Prefer:

- “truth loses practical rule in the moment”;
- “the sinner has practiced yielding”;
- “conscience may become less sensitive.”

Avoid:

- “brain switched off” as explanation;
- “he now believes pornography is normal” as universal stage.

## Forbidden

- permanent-seared-conscience diagnosis;
- declaring person beyond repentance.

## Transition

> At this point the public Christian role can remain long after the hidden life has become habitual. That raises the most dangerous diagnostic question: what does the contradiction say about faith itself?

---

# 8. Section — “Лицемер, павший верующий или ещё не обращённый?”

## Governing question

Can parents or pastors infer regeneration status directly from the discovered sin?

## Primary claim

> Scripture requires serious self-examination and recognizes false profession, but it also recognizes grievous falls in true believers; observers must judge fruit/conduct faithfully without pretending to read God’s secret decree.

## Biblical/confessional center

- 1 John 1:5–10;
- Matt. 7 fruit texts may be considered with context;
- 1689 ch. 17;
- 1689 ch. 18 if assurance needs brief mention.

## Required contrast

`recurring fall + confession + warfare + help-seeking`

is not identical to

`planned concealment + defense + recruitment + refusal of correction + settled peace with sin`.

## Forbidden

- “true Christian teenager never does X”;
- “past prayer/baptism guarantees conversion”;
- “one relapse proves false conversion.”

## Transition

> Whatever the final diagnosis of the heart, the immediate biblical alternative to darkness is not image repair. It is light.

---

# 9. Section — “Выйти на свет”

## Governing question

What is the first direction of escape before Part II discusses parents, boundaries, and repentance in detail?

## Primary claim

> The double life begins to break where protected secrecy is surrendered to truth, confession, exhortation, and real help—and Christian hope rests finally in Christ, not in successful exposure.

## Biblical center

- 1 John 1:5–2:2;
- Heb. 3:13.

## Part I scope

Do not yet give the full parent response matrix.

Name:

- truth;
- confession;
- help;
- refusing isolation;
- taking temptation seriously early;
- Christ the righteous Advocate / cleansing.

Then send reader to Part II.

## Required gospel note

The article must not end Part I as though exposure itself saves.

Exposure, consequences and accountability do not atone. Christ is not merely a better accountability system.

## Transition to Part II

> If truth must come into the light, parents face the question they cannot avoid: how do you build a home where truth is confronted seriously without teaching the child that lying is safer than confession—and what can parents do if they cannot give a new heart?

---

# 10. Optional safety sidebar — “Not every sexual digital case is voluntary sin”

## Primary claim

A child may be:

- foolish/sexually sinful in one action;
- pressured or coerced inside a peer/romantic relationship;
- subjected to nonconsensual redistribution;
- extorted, groomed, stalked, threatened, or otherwise victimized later.

These moral categories must not be collapsed.

## Modern anchors

Core official safety evidence:
- UNICEF 2026;
- NCMEC sextortion reports.

Optional 2026 context:
- 49-study cyber dating violence systematic review/meta-analysis, PMID 39283366;
- U.S. national teen sexting/nonconsensual-sharing/sextortion study, N=3,466 ages 13–17, PMID 41653178.

## Best placement

Sidebar/callout, not full main section unless narrative naturally requires it.

## Required wording

> Victimization is not transformed into the child’s guilt because the child previously acted foolishly or sinfully.

And:

> A romantic relationship does not turn pressure, threats, stalking or nonconsensual sharing into consent.

## Fact-finding guard

If a real case is being discussed, distinguish:

- creation of the material;
- voluntary vs pressured/coerced sending;
- scope of consent;
- later redistribution;
- threats/blackmail/control;
- age/identity of the other person;
- current safety risk.

Do not turn this into graphic amateur forensic interrogation.

## Forbidden

- using UNICEF “1 in 5” as a statistic for voluntary sexual behavior;
- treating every sexting case as coerced;
- exact cyber-dating prevalence/effect claims while the research sources retain `LOCATOR_HOLD`.

---

# 11. Part I argument in one paragraph

This is **not final prose**; it is the editorial logic test:

> The hidden adolescent life begins with no new anthropology: the fallen heart desires what God forbids, and sin can be genuinely sweet for a moment while hiding death. Companionship gives those desires a social world, and the digital age gives that world unusual persistence, privacy, access, and concealment; in the AI era, even private relationship-like conversation no longer always requires another human to be online. Pornography and sexualized communication become especially powerful not because devices manufacture depravity but because they place fitting occasions beside existing lust. Biblical sexual ethics therefore must name pornography severely without teaching that the body, attraction, desire, or marital sexuality are themselves dirty: pornography corrupts a good created gift by severing it from God’s order and turning persons into objects of lust and use. To preserve the pleasure, the child may learn to manage truth—what parents see, what church sees, what peers see. Sin’s deceit can then harden him until correct doctrine remains in memory while it exerts little practical rule in the moment. Yet observers must not confuse every grievous fall with final apostasy, and they must not confuse a child’s voluntary sin with another person’s later coercion or abuse. Scripture calls sinners away from darkness and into the light, where Christ—not exposure—is the refuge. Part II must then ask what parents are responsible to do with that truth.

If future prose cannot preserve this chain, revise the outline before drafting.

---

# 12. Section redundancy guard

Do not repeat:

- fallen-heart thesis in every digital paragraph;
- “internet is not root” more than 2–3 strategic times;
- clinical caveats after every pornography mention;
- false-profession guard in every hardening paragraph;
- modern statistics in theological sections;
- AI as a new theme in multiple sections;
- coercion/safeguarding details outside the safety sidebar unless directly necessary;
- positive-sexuality guard so often that it derails Part I into a marriage article.

Each distinction gets one primary home.

---

# 13. Part I pre-draft readiness verdict

**The argument is specific enough to move to product preflight and drafting. Broad Research is closed.**

Still required before/while writing product prose:

1. live product owner-lane / open-PR overlap check;
2. re-read current product content/source rules at exact `main`;
3. selected exact Bible translation/reference policy (`105/107` controls; current product authority wins if changed);
4. exact short Brooks/Owen quotation(s) chosen and reverified only if actually retained;
5. exact strongest pornography systematic/longitudinal source chosen if a modern outcome claim survives;
6. exact locator reopening for any numerical AI/coercive claim that survives (`LOCATOR_HOLD` must be cleared first);
7. verify that reader-facing Part I contains the concise positive sexual-ethics counterweight required in §5;
8. final title/slug after current product taxonomy review;
9. final Gospel-center/red-team using `131_CROSS_PART_GOSPEL_CENTER...` and late P0 controls.

No other broad psychology/digital-risk pass is required for Part I.
