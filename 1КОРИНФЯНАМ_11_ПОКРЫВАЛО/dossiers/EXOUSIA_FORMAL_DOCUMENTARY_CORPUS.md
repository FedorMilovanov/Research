...
The prior state `PP_PREPOSITIONS_NOT_ENUMERATED` is superseded at **document level**, but not by pretending the target runtime read the XLSX binary.
...
FENDEL_EXOUSIAN_XLSX = OFFICIAL_DATASET_FILE_OBJECT_CLOSED / BINARY_TERMINAL_ORA_TRANSPORT_HOLD
...
NEW_DIRECT_XLSX_BINARY_OR_AUTHORIZED_PKNT_PAGE_ACCESS = REOPEN_TRIGGER
```
