...
FENDEL_ROMAN_PP_PREPOSITIONS = ENUMERATED_2026_08_10 / APO_GEN_BGU_7_1655 / KATA_GEN_P_OXY_8_1120 / EIS_ACC_P_OXY_9_1205
FENDEL_ROMAN_PP_IS_EPI_GEN = FALSE (none of the three Roman-period PP rows is ἐπί + genitive)
FENDEL_EXOUSIAN_XLSX_OBJECT = CLOSED_OFFICIAL_ORA
FENDEL_EXOUSIAN_XLSX_CONTENT = READ_DIRECT_2026_08_10_VIA_ORA_FILE_RENDER (full 9-chunk table scan)
FENDEL_EXOUSIAN_XLSX_BINARY = TERMINAL_ORA_TRANSPORT_HOLD_CURRENT_RUNTIME (original .xlsx bytes)
```

Never turn `PP=3` into `ἐπί/gen.=3` before reading the rows — **rows now read**: (1) BGU.7.1655 `ἐξουσίαν ἕξει ἀπὸ τῶν κληρονόμων μου` (ἀπό+gen), (2) P.Oxy.8.1120 `μὴ ἔχων κατ' αὐτῆς ἐξουσίαν` (κατά+gen), (3) P.Oxy.9.1205 `μηδεμίαν τε ἐξουσίαν ἔχειν εἰς αὐτοὺς ἀπὸ τῆς ἐνεστώσης ἡμέρας` (εἰς+acc). No `ἐπί + genitive` row exists among the three Roman-period PP instances of `ἐξουσίαν ἔχειν` in the dataset.
...
FENDEL_EXOUSIAN_XLSX = OFFICIAL_DATASET_CONTENT_READ_DIRECT_2026_08_10 / PP_PREPOSITIONS_ENUMERATED_APO_KATA_EIS / NO_EPI_GEN / BINARY_ORIGINAL_TERMINAL_ORA_TRANSPORT_HOLD
