...
# 2. Modern large-quotation / refutation family
...
## 2.2 Costa 2023/2024 — complete official PDF CLOSED DIRECT
...
- official PDF route exposed by the journal download object.

The journal abstract itself directly states the core result: antagonistic Pauline and Corinthian points of view are identified and **vv4–9 are quotations not endorsed by Paul**.

The complete official 43-page PDF was independently rendered/read in the target workflow. Body-level speaker map:
...
Body-level interpretive details relevant to adversarial testing:
...
Costa is now a directly read argument, not merely an abstract-level model node. The project's independent objections remain live: unmarked boundary, local causal cohesion, v10 `διὰ τοῦτο`, same-letter controls and early reception.
...
```text
vv2_3 = PAUL
vv4_5 = CORINTHIAN_VIEW_QUOTED_BACK
v6 = PAUL_REDUCTIO
vv7_10 = CORINTHIANS
v11 = PAUL_RESUMES
v14 = IRONIC_OR_SARCASTIC_REUSE_IN_HER_MODEL
v15 = HAIR_AS_COVERING_TRUMP_CARD_IN_HER_MODEL
v16 = DENIAL_OF_PROPOSED_CUSTOM_IN_HER_MODEL
```
...
```text
v3 = HIERARCHICAL_TRADITION_USED_MODIFIED_BY_PAUL
vv11_12 = INTERDEPENDENT_TRADITION_USED_MODIFIED_BY_PAUL
SPEAKER_CHANGE = NOT_REQUIRED_BY_MODEL
```

## 3.2 Newberry 2019

Julie Newberry, “Paul's Allusive Reasoning in 1 Corinthians 11.7–12,” *NTS* 65.1 (2019): 43–58. DOI `10.1017/S0028688518000292`.

Provides a coherent one-speaker route from vv7–9 through v10 to vv11–12.

```text
REAL_TENSION != MULTIPLE_SPEAKERS_REQUIRED
```

## 3.3 Kowalski 2020 — second official institutional route
...
Official KUL institutional item:
- https://repozytorium.kul.pl/items/5478d348-a52e-4709-8b75-b6ef407edff1
- handle `20.500.12153/3396`
- file object `Kowalski_Marcin_Miedzy_darem_Bozym_a_konstruktem_spolecznym.pdf`, 742.32 KB.

The institutional abstract directly closes the macrostructure:

```text
v2 = INTRODUCTION
v3 = PAULINE_THESIS
vv4_6 = CULTURAL_ARGUMENT
vv7_12 = CHRISTOLOGICAL_THEOLOGICAL_ARGUMENT
vv13_15 = NATURAL_LAW_ARGUMENT
FRAME = ECCLESIAL_INTRODUCTION_AND_CONCLUSION
```

```text
KOWALSKI_CONTINUOUS_PAULINE_STRUCTURE = CLOSED_DIRECT_INSTITUTIONAL_ABSTRACT
KOWALSKI_V3_AS_PAULINE_THESIS = CLOSED_DIRECT_INSTITUTIONAL_ABSTRACT
KOWALSKI_KUL_ITEM = CLOSED_DIRECT_INSTITUTIONAL
KOWALSKI_KUL_PDF_FILE_OBJECT = CLOSED_DIRECT_INSTITUTIONAL
KOWALSKI_FULL_PDF_BODY_TARGET_RUNTIME = STILL_RENDER_HOLD
KOWALSKI_EXACT_V10_ANGELS_DETAILS = NOT_DIRECTLY_VERIFIED_IN_TARGET_WORKFLOW
```
...
# 4. Same-letter quotation / slogan controls

Primary text has explicit speech/source frames:

```text
1COR_1_12 = λέγω δὲ τοῦτο ὅτι ἕκαστος ὑμῶν λέγει
1COR_15_12 = πῶς λέγουσιν ἐν ὑμῖν τινες ὅτι...
1COR_7_1 = Περὶ δὲ ὧν ἐγράψατε
```
...
Classic unmarked slogan candidates include 1 Cor 6:12–13, 8:1/4 and 10:23, but ownership/boundaries are disputed.
...
1 Cor 10:23 is a short maxim plus immediate `ἀλλά` qualification, not a verified six/eight-verse hostile internally explanatory block.
...
Duane Watson and Martin Culy, *Quoting Corinthians* (2018): publisher + lawful preview close the method/front portion and state application to eleven passages, but no direct 1 Cor 11 application is verified.
...
Paul A. Holloway 2021 supplies another methodological control:
...
No close same-letter analogue is verified in the bounded audit for a six/eight-verse opponent block without source frame but with internal `γάρ / καὶ γάρ` cohesion followed by Paul's own `διὰ τοῦτο` response.

```text
NO_CLOSE_PARALLEL_FOUND != PAUL_COULD_NEVER_DO_IT
```

Costa-specific burdens after direct full-body reading:
...
COSTA_FULL_BODY_INTERNAL_COHERENCE = REAL
COSTA_INTERNAL_COHERENCE != BOUNDARY_PROOF
```
...
Checked major witnesses read the passage continuously:

```text
TERTULLIAN = CONTINUOUS_PAULINE_OWNERSHIP
CHRYSOSTOM = CONTINUOUS / DIA_TOUTO_LOCAL_BACKLINK
AMBROSIASTER = VV8_10_CONTINUOUS
...
Early reception is reception evidence, not proof of original authorial intent.
...
3. proposed blocks are longer/more explanatory than safer slogan candidates, or require repeated unmarked transitions;
...
Direct reading of a model can improve how accurately the model is represented without changing the probability assigned to its proposed speaker boundary.

---

# 9. Acquisition disposition / multilingual reopen

```text
COSTA_2023_2024_FULL_OFFICIAL_PDF = CLOSED_DIRECT
