# 1 Коринфянам 11:4–5 — ritual/divination/prophetic head-state dossier

**Статус:** `EVERGREEN-DOSSIER / FEMALE-MANTIC-PERSONNEL / RITUAL-HEAD-STATE / OBJECT-IDENTITY / RESEARCH-ONLY / PUBLICATION-HOLD`  
**Последняя проверка:** 2026-08-10

## 0. Authority rule

1 Cor 11:4–5 regulates the head **during prayer/prophecy**. This dossier owns the comparative question:

> What direct evidence links female mantic/prophetic personnel, ritual performance, hair/headwear and material/visual head-state?

Exegetical grades remain in `00ZZZZZZZZZZZZZZZZZZ_CURRENT_CLAIM_REGISTRY_2026-08-10.md`.

Future discoveries update this dossier; do not create a `pass_N` / Z-successor.

---

# 1. Fail-closed boundaries

```text
FEMALE_PROPHETIC_ROLE_ATTESTED != HEAD_OR_HAIR_CODE_ATTESTED
RITUAL_HEAD_HAIR_REGULATION != PROPHETESS_HEAD_HAIR_REGULATION
PRIESTESS_ORACLE_FUNCTION != FORMAL_PROPHETIS_TITLE
LITERARY_ECSTATIC_HAIR != FIRST_CENTURY_CORINTHIAN_PRACTICE
VISUAL_HONOUR != SURVIVING_IDENTIFIED_PROPHETESS_PORTRAIT
SAME_LOCALITY != SAME_OBJECT_OR_PERSON
ADJACENT_STELE_ICONOGRAPHY != TARGET_INSCRIPTION_ICONOGRAPHY
PHI_ORDINAL != AUTOMATICALLY_REHM_OR_CURRENT_EDITION_NUMBER
WORD_PROPHETIS_IN_AN_INSCRIPTION != FEMALE_PERSON_AUTOMATICALLY
REFERENCE_LIST_ENTRY != VERIFIED_PROPHETESS_OBJECT_AUTOMATICALLY
SEQUENTIAL_EDITOR_NUMBERING != PROOF_OF_OBJECT_CROSSWALK
CULT_CONTEXT != PROPHETIS_NOUN_BEARER
PRINT_REFERENCE_ERROR != LICENSE_TO_GUESS_THE_INTENDED_REFERENCE
PDF_TEXT_LAYER_READ != PAGE_IMAGE_AUTOPSY
ROMAN_GREEK_JEWISH_CHRISTIAN_COMPARAND != EXACT_CORINTH_TRIGGER
NEGATIVE_BOUNDED_SEARCH != GLOBAL_NONEXISTENCE
```

Every new corpus-list item must pass **noun-bearer / syntax / object-identity audit** before entering female-person prosopography.

---

# 2. Current evidence matrix

| Node | Evidence status | Safe use | Same-object head/hair? |
|---|---|---|---|
| Tryphosa, Didyma | direct `προφῆτις`; PHI/SEG/current-edition crosswalk | strongest near-Pauline Didyma woman | **No** |
| Eirene, Miletos/Didyma | direct `πρόμαντις` | female mantic title | **No** |
| Cleopatra, Didyma | restored `[προφ]ήτιδι` | fragmentary candidate | **No** |
| Didyma 235B | `καλὴ προφητίσκη` graffito | informal/ironic language; office unproved | **No** |
| Agatho, Didyma | prophet’s wife + building benefaction | source-error negative control | **No** |
| Potta, Maionia | `TAM V.1 535`, `προφῆτιν σώτειραν` | external Asia-Minor prophetis | **No** |
| Ammias, Thyateira | direct priestess + vision-oracle formula | oracular priestly control, **not prophetis** | **No** |
| Ammia, Philadelphia | Eusebian female prophetess | early-Christian reception | **No** |
| Nanas, Phrygia | PHI-indexed object + Poirier 2004 direct specialist body printing full epitaph | late female prophetess object | **No marker found** |
| Apphe, Kalchedon | `IK Kalchedon 61 = CIG 3796`; LSJ/CIG + prosopography | Roman Apollo prophetis | **No marker found** |
| Miletos 481 | women among divine-manifestation channels | female divine information | **No** |
| Eresos / IG XII Suppl.126 | direct PHI sanctuary regulation excepts priestess + prophetess from female temple-entry ban | pre-Pauline institutional office | **No named woman** |
| Thera / IG XII.3 1349/863 | Delphic `[προφ]ῆτις` sends oracle | anonymous epigraphic control | **No** |
| IGLSyr 1 51 | `ἀναγραφή` itself called `προφήτις` | semantic false-positive control | **Not a woman** |
| Athens / IG II² 13148 | direct funerary epigram; official IG organization + text conflict with prophetess list | very likely citation/reference error | **Not countable as prophetess** |
| Nisyra / SEG 49.1624 | PHI/TM object pinned; Malay no.137 Nisyra/Roman-soteria context independently verified, exact crosswalk/body still open | candidate-list HOLD narrowed | **Unknown** |
| Termessos / TAM III,1 870 | object mapping + independent Demeter Eleusinia cult-reference control; body incomplete | candidate-list HOLD narrowed | **Unknown** |
| Lebadeia / IG VII 3111 | deterministic object mapping; false-neighbor identities firewalled; body incomplete | candidate-list HOLD | **Unknown** |
| Andania / Lycosura | direct ritual hair/headwear regulation | separate regulatory axes | **Yes, but not prophetess** |
| Virgil / Lucan | prophetic head/hair imagery | literary comparison only | **Literary only** |

Current bounded result:

```text
DIRECT_NEAR_PAULINE_PROPHETESS_HEAD_HAIR_CODE = NOT_VERIFIED
IDENTIFIED_TRYPHOSA_EIRENE_PORTRAIT = NOT_FOUND
GLOBAL_NONEXISTENCE = NOT_CLAIMED
```

---

# 3. Didyma / Miletos — core female mantic evidence

## 3.1 Tryphosa — direct `προφῆτις`

One inscription appears under different numbering systems:

```text
PHI_OBJECT_ID = 247445
PHI_INTERFACE_ORDINAL = Didyma 450
SEG_REFERENCE = SEG 30.1286
CURRENT_DIDYMA_REFERENCE = Didyma 708
PHI Didyma 450 = SEG 30.1286 = Didyma 708
PHI_ORDINAL_450 != EDITION_ITEM_450
```

Greek includes:

```text
Πλαταινὶς Μέλανος ἡ καλουμένη
Τρυφῶσα μήμης προφήτιδος
Τρυφώσης ...
```

Primary:
- https://epigraphy.packhum.org/text/247445

```text
TRYPHOSA_PROPHETIS = A_EPIGRAPHIC
TRYPHOSA_CROSSWALK_SEG30_1286_DIDYMA708 = STRONG_EXPLICIT_SPECIALIST
TRYPHOSA_DATE_NEAR_EARLY_FIRST_CENTURY = B_HIGH_PROSOPOGRAPHIC
TRYPHOSA_HEAD_HAIR_CODE = NOT_ATTESTED
```

## 3.2 Eirene — direct `πρόμαντις`

`Milet VI,2 546`:

```text
Εἰρήνη
πρόμαντι χαῖρε.
```

Primary:
- https://inscriptions.packhum.org/text/351272

```text
EIRENE_PROMANTIS = A_EPIGRAPHIC
EIRENE_DIDYMA_ORACLE_AFFILIATION = B_HIGH_CONTEXTUAL
EIRENE_DATE = BROAD_IMPERIAL
EIRENE_HEAD_HAIR_CODE = NOT_ATTESTED
```

## 3.3 Cleopatra — fragmentary restoration + numbering firewall

Modern literature cites `I.Didyma 273`; PHI exposes the matching fragment under interface ordinal `Didyma 533`:

```text
[προφ]ήτιδι [τοῦ Διδυμέως?]
[Κλεο]πάτρᾳ ...
```

PHI ordinal `Didyma 273` is a different underlying inscription.

```text
REHM_IDIDYMA_273 = CLEOPATRA_FRAGMENT
PHI_ORDINAL_DIDYMA_533 = SAME_TEXT_BY_UNIQUE_TEXT_IDENTITY
CLEOPATRA_PROPHETIS = B_C_FRAGMENTARY_RESTORATION
CLEOPATRA_HEAD_HAIR_CODE = NOT_ATTESTED
```

## 3.4 Didyma 235B — `προφητίσκη`

Published text:

```text
Γάϊς εὐχαριστῶ
τῇ καλῇ προφητίσκῃ
```

Nawotka cannot determine whether it is affectionate or ironic; specialist JHS discussion describes the graffito as sarcastic.

```text
DIDYMA_235B_TEXT = CONTROLLED_TRANSCRIPTION
PROPHETISKE = HAPAX_IN_NAWOTKA_CONTROL
FORMAL_PROPHETIC_OFFICE = NOT_ESTABLISHED
DIRECT_STONE_OR_REHM_PAGE_AUTOPSY = HOLD
HEAD_HAIR_CODE = NOT_ATTESTED
```

## 3.5 Agatho — inscription overrides prose compression

Didyma 302+303 identifies Agatho as the wife (`σύμβιος`) of the man prophesying and records roofing/plastering associated with the prophetic building.

```text
DIDYMA_302_303_AGATHO_WIFE_OF_PROPHET = A_TEXT
DIDYMA_302_303_BUILDING_BENEFACTION = A_B_HIGH
DIDYMA_302_303_SUPPORTS_AGATHO_AS_PROPHETIS = FALSE
AGATHO_INDEPENDENT_PROPHETIS_SOURCE = NOT_VERIFIED
```

## 3.6 Miletos 481 / I.Didyma 496

Primary text says manifestations occur:

```text
διὰ παρθένων καὶ γυναικῶν
...
δι’ ἀρρένων καὶ νηπίων
```

Primary:
- https://epigraphy.packhum.org/text/252571

```text
FEMALE_DIVINE_MANIFESTATION_CHANNELS = A_PRIMARY_TEXT
FORMAL_FEMALE_PROPHETIS_TITLE = NOT_THE_POINT
DATE = SECOND_CENTURY_CE_SPECIALIST_CONTROL
HEAD_HAIR_CODE = NOT_ATTESTED
```

---

# 4. External named female mantic personnel

## 4.1 Potta of Maionia — `TAM V.1 535`

Published transcription:

```text
Ἑρμογένης Μητροδώρου Διὶ ᾿Αρίου κατ’ ἐπιταγὴν
σωτηρίας ἕνεκεν τῆς ἐκ τοῦ Διὸς
Ποτταν Μενε[κ]ρ̣άτ̣ου προφῆτιν σώτειραν
γενομένην τοῦ Ἑρμογένου
```

```text
REFERENCE = TAM V.1 535
LOCATION = MAIONIA_LYDIA
PHI_OBJECT_ID = 263959
DIRECT_PHI_BODY_CURRENT_RUNTIME = CACHE_HOLD
```

Independent Oxford scholarship identifies the same woman as prophetess / `soteira` and discusses her as mediator or mouthpiece of Zeus.

```text
POTTA_PROPHETIS = B_HIGH_EXACT_REFERENCE_PLUS_INDEPENDENT_SCHOLARLY_CONTROL
POTTA_SOTEIRA = B_HIGH
POTTA_DATE = UNKNOWN
POTTA_HEAD_HAIR_CODE = NOT_ATTESTED
```

Nearby `TAM V.1 533` has been described as possibly showing a veiled figure, but it is **a different object/person**.

```text
MAIONIA_533_POSSIBLY_VEILED != POTTA_TAM535
```

## 4.2 Ammias of Thyateira — direct priestess/oracle function, not prophetis

`TAM V,2 1055` directly identifies Ammias as:

```text
ἱερείᾳ τῶν θεῶν
```

and directs truth-seekers to her altar, with an answer received:

```text
διὰ ὁράματος νυκτὸς καὶ ἡμέρας
```

```text
AMMIAS_THYATEIRA_TAM_V2_1055 = A_EPIGRAPHIC_DIRECT
AMMIAS_PRIESTESS = A_TEXT
AMMIAS_POSTMORTEM_TRUTH_VISION_ORACLE_FUNCTION = A_B_HIGH_DIRECT_TEXT
AMMIAS_FORMAL_PROPHETIS_TITLE = NOT_PRESENT
AMMIAS_HEAD_HAIR_CODE = NOT_ATTESTED
AMMIAS_THYATEIRA != AMMIA_PHILADELPHIA
```

## 4.3 Ammia of Philadelphia — early-Christian literary prophetess

Eusebius, *HE* 5.17, preserves an anti-Montanist source naming `Ammia in Philadelphia` among recognized prophetic precedents.

```text
AMMIA_PHILADELPHIA = DIRECT_EUSEBIAN_LITERARY_RECEPTION
FEMALE_PROPHETIC_WITNESS = STRONG_EARLY_CHRISTIAN
EPIGRAPHIC_OBJECT = NOT_CLAIMED
HEAD_HAIR_CODE = NOT_ATTESTED
```

## 4.4 Nanas of Phrygia — text-body closed, object image still open

PHI indexes the object as Haspels, *Highlands of Phrygia* 338,107 (`PH271564`) and preserves the title/name sequence:

```text
προφήτισα
Νανας Ἑρμογένου
εὐχῆς καὶ λιτανίης
```

John C. Poirier, “The Montanist Nature of the Nanas Inscription (Steinepigramme 16/41/15),” *Epigraphica Anatolica* 37 (2004): 151–159, is directly accessible from the University of Cologne / *Epigraphica Anatolica* archive. The article body explicitly calls the object a fourth-century epitaph from southeast of Kotiaeion and **prints the complete inscription**. It independently confirms that the deceased is called `prophetissa` and names Nanas daughter of Hermogenes. Poirier points to Tabbernee 1997, pp.420–421 and fig.77 for the edited text / epigraphic and visual details, and to Merkelbach–Stauber 2001 no.16/41/15.

Direct specialist body:
- https://ifa.phil-fak.uni-koeln.de/fileadmin/IfA/EpiAna_pdfs/037151_Poirier_The_Montanist_Nature_of_the_Nanas_Inscription__Steinepigramme_16-41-15_.pdf

The runtime obtained the official PDF text layer, but the PDF page screenshot route cache-missed. Therefore **body-text custody is closed; page-image / monument-image custody is not**.

Poirier argues for Montanist identity, while explicitly rehearsing Trevett’s challenge and the wider classificatory uncertainty in fourth-century Phrygia. For this dossier, sectarian identity remains separate from the epigraphic office-title fact.

```text
NANAS_PHI_OBJECT = 271564
NANAS_MONUMENT = EPITAPH_TOMBSTONE
NANAS_DATE = FOURTH_CENTURY_CE_IN_DIRECT_POIRIER_BODY
NANAS_POIRIER_2004_DIRECT_FULL_BODY_TEXT_LAYER = CLOSED
NANAS_COMPLETE_INSCRIPTION_PRINTED_IN_POIRIER = DIRECT_SPECIALIST
NANAS_PROPHETIS = A_B_HIGH_DIRECT_OBJECT_PLUS_DIRECT_SPECIALIST_BODY
NANAS_RELIGIOUS_IDENTITY = DISPUTED / POIRIER_ARGUES_MONTANIST
NANAS_CHRONOLOGICAL_RELEVANCE_TO_PAUL = LOW_LATE
NANAS_SAME_OBJECT_HEAD_HAIR_MARKER = NOT_FOUND
NANAS_PDF_PAGE_IMAGE_AUTOPSY = RUNTIME_CACHE_HOLD
TABBERNEE_FIG77_OBJECT_IMAGE = HOLD
```

Do not substitute generic Phrygian funerary stelae for Nanas’s own monument.

## 4.5 Apphe of Kalchedon — Apollo Chresterios prophetis

Specialist prosopography identifies:

```text
NAME = Ἄπφη
ROLE = προφῆτις OF APOLLO_CHRESTERIOS
REFERENCE = IK Kalchedon 61 = CIG 3796
OBJECT_TYPE = FUNERARY_INSCRIPTION
DATE = ROMAN_PERIOD
RELATION = PATRONA_OF_ORBANILLA / ORBANILLA_AS_THREPTE
```

Independent lexical control cites **CIG 3796 (Chalcedon)** as an epigraphic example of `προφῆτις`. Independent historical/prosopographic control places CIG 3796 among Roman-period Kalchedon inscriptions and identifies `Ἄπφη` / genitive `Ἄπφης`. Current specialist prosopography explicitly identifies her as prophetis of Apollo Chresterios and crosswalks `IK Kalchedon 61 = CIG 3796`.

Official CIG custody is pinned to *Corpus Inscriptionum Graecarum* II.2, Bithynia, which covers CIG 2878–3809; Digimom exposes the official full-volume PDF, but its binary could not be retrieved in the current runtime. Therefore the old-edition body is still not personally autopsied.

```text
APPHE_CIG3796_PROPHETIS_LEXICON_CONTROL = INDEPENDENT_REFERENCE
APPHE_IK61_CIG3796_CROSSWALK = STRONG_SPECIALIST
APPHE_CIG3796_ROMAN_KALCHEDON_CONTROL = INDEPENDENT_HISTORICAL
APPHE_PROPHETIS = B_HIGH_MULTISOURCE_CORPUS_CONTROL
APPHE_CIG_OFFICIAL_VOLUME_IDENTITY = CLOSED
APPHE_DIRECT_CIG3796_PAGE_BODY = HOLD_BINARY_RUNTIME
APPHE_HEAD_HAIR_CODE = NOT_FOUND
```

Do not promote Apphe to direct-page `A_EPIGRAPHIC` until CIG 3796 / IK Kalchedon 61 itself is opened.

---

# 5. `prophetis` candidate-list syntax audit

Nawotka footnote 77 supplies **ten external inscription references** as comparanda for prophetesses. The list is useful for discovery but cannot be treated as ten verified historical women.

```text
NAWOTKA_FN77_TEN_REFERENCES = AUTHOR_LIST_COUNT
TEN_REFERENCES != TEN_VERIFIED_HISTORICAL_PROPHETESSES
REFERENCE_LIST_ENTRY_REQUIRES_NOUN_BEARER_AUDIT = true
```

## 5.1 IGLSyr 1 51 — decisive false-person control

Direct PHI text, Commagene, 64–ca.38 BCE:
- https://epigraphy.packhum.org/text/242124

```text
δαιμονίωι δὲ γνώμηι ταύτην ἀναγραφὴν εὐσεβείας προφήτιν ἐποιησάμην
```

The grammatical bearer of `προφήτιν` is **`ἀναγραφή`**, the inscription itself.

```text
IGLSYR_1_51_PROPHETIS = PERSONIFICATION_OF_INSCRIPTION
IGLSYR_1_51_FEMALE_PROPHETESS = FALSE
FOOTNOTE_REFERENCE_LIST != FEMALE_PROSOPOGRAPHY
```

## 5.2 Thera — `IG XII,3 1349/863`

PHI exposes:

```text
χρησμὸν ἔπεμψε θεοῦ Δελφοῖσι [προφ]ῆτις ...
```

Safe use: epigraphic mention of an anonymous Delphic prophetess transmitting an oracle. It is **not** a named Theran female-office prosopography entry and supplies no head-state.

## 5.3 Eresos / Lesbos — direct pre-Pauline institutional office

Direct PHI:
- https://epigraphy.packhum.org/text/75479

`IG XII Suppl.126` is dated by PHI to the second century BCE. Lines 18–20 directly regulate temple access:

```text
μὴ εἰστείχην δὲ μηδὲ γ̣ύ[ναικ]α
εἰς τὸν ναῦον ... πλὰν τᾶς ἰρέας
καὶ τᾶς προφήτιδος.
```

Source genealogy:

```text
IG_XII_SUPPL_126
= SCHWYZER_DGE_633
= LSCG_124
= ERESOS_SANCTUARY_REGULATION
```

This is direct pre-Pauline evidence for an institutional female prophetic office inside ritual-space regulation.

```text
ERESOS_PROPHETESS_OFFICE = A_EPIGRAPHIC_DIRECT_PHI
ERESOS_DATE = SECOND_CENTURY_BCE_IN_PHI
ERESOS_PROPHETESS_NAMED_WOMAN = NO
ERESOS_HEAD_HAIR_CODE = NO
```

## 5.4 Athens — `IG II² 13148 / PH15638`: very likely citation/reference error

Nawotka footnote 77 lists `IG II² 13148` among external prophetess-inscription comparanda. Direct PHI indexing previously inspected in this audit exposes a short female funerary epigram whose preserved lines concern the deceased woman, her stele, her failure to enter the marriage chamber and grief left to her nurses; no `προφῆτις` title or mantic vocabulary is present in that indexed text.

Independent controls now tighten the classification:

- the official Berlin *Inscriptiones Graecae* index places `IG II² 13148` inside the long `tituli sepulcrales` sequence;
- independent funerary-epigram scholarship also classifies `IG II² 13148` as a funerary epigram;
- the direct PHI-indexed text itself is funerary rather than mantic.

The converging controls make the footnote’s exact number **very likely a citation/reference error**. They do **not** reveal which inscription was intended.

```text
IGII2_13148_PHI_OBJECT = 15638
IGII2_13148 = DIRECT_PHI_INDEXED_FEMALE_FUNERARY_EPIGRAM
IGII2_13148_OFFICIAL_IG_CLASS = TITULUS_SEPULCRALIS_SEQUENCE
IGII2_13148_PROPHETIS_IN_INDEXED_TEXT = ABSENT
NAWOTKA_FN77_IGII2_13148 = VERY_LIKELY_REFERENCE_ERROR
NAWOTKA_INTENDED_REFERENCE = UNKNOWN
COUNT_AS_SECURE_PROPHETESS = NO
```

Do not silently repair the citation to a neighboring inscription. Only an explicit erratum, source manuscript/notes, or independently identifiable intended object can close the intended-reference question.

## 5.5 Nisyra — `SEG 49.1624`: object pinned, contextual substrate narrowed

```text
PHI_OBJECT = 348429
TRISMEGISTOS_ID = TM 949255
LOCATION = NISYRA_LYDIA
DIRECT_BODY_CURRENT_RUNTIME = CACHE_HOLD
```

The PHI/search surface exposes the opening dedication formula beginning `Θεῷ Βασιλεῖ ...`; noun-bearer remains unresolved.

Independent object/corpus work narrows the acquisition target:

- direct PHI for adjacent `SEG 49.1623` identifies it as H. Malay, *Researches in Lydia, Mysia and Aiolis* (1999), p.128, **no.136**, Nisyra, 48/47 BCE;
- independent Oxford scholarship cites **Malay no.137 (Nisyra, Roman imperial period)** as a `σωθεὶς ἐκ (μεγάλου) κινδύνου` / soteria-type inscription;
- Malay 1999 is independently identified as ETAM 23, the relevant Nisyra corpus publication.

Because no.136 is `SEG 49.1623`, it is tempting to identify sequential no.137 with `SEG 49.1624`. That crosswalk is **plausible but not yet direct** and is therefore deliberately not promoted.

Adjacent `SEG 49.1623` is a **different marble stele** with olive branch and double axe. None of its text or iconography may be transferred to 1624.

```text
MALAY_1999_NO136 = SEG49_1623_DIRECT_PHI
MALAY_1999_NO137 = NISYRA_ROMAN_IMPERIAL_SOTERIA_INSCRIPTION_INDEPENDENT_CONTROL
MALAY_NO137_EQ_SEG49_1624 = PLAUSIBLE_NOT_DIRECTLY_VERIFIED
SEG49_1623_ICONOGRAPHY != SEG49_1624_ICONOGRAPHY
NISYRA_1624_PROPHETIS_NOUN_BEARER = HOLD_UNTIL_BODY
NISYRA_1624_HEAD_STATE = UNKNOWN
```

The contextual substrate is now stronger, but the prophetess claim itself is **not** upgraded.

## 5.6 Termessos — `TAM III,1 870`: cult context verified, noun-bearer still open

```text
TERMESSOS_TAMIII1_870_PH280975 = DETERMINISTIC_CORPUS_MAPPING
TERMESSOS_TAMIII1_870 = BODY_AND_SYNTAX_HOLD
```

Independent scholarship supplies a useful but limited control. Hugh Bowden’s 2007 study of Demeter Eleusinia cult transmission lists **Termessos — TAM III 870** in its evidence table for a local cult of **Demeter Eleusinia**. Separate epigraphic literature also cites TAM III 870 in Termessian personal-name/cult discussion, showing that the object is not merely a floating number.

This validates the **cultic relevance and reference identity** of 870, but does not show the full Greek text or establish who bears `προφῆτις`.

```text
TERMESSOS_870_DEMETER_ELEUSINIA_CULT_CONTEXT = VERIFIED_INDEPENDENT_SCHOLARLY
TERMESSOS_870_REFERENCE_NOT_RANDOM = true
TERMESSOS_870_PROPHETIS_NOUN_BEARER = HOLD
TERMESSOS_870_OBJECT_TYPE = HOLD
DO_NOT_COUNT_AS_NAMED_FEMALE_PERSON_YET
```

Do not infer from neighboring TAM numbers that 870 is a theatre-seat inscription; numbering adjacency is not object-identity proof.

## 5.7 Lebadeia — `IG VII 3111`: identity firewall strengthened

```text
IGVII_3111_PH146626 = DETERMINISTIC_CORPUS_MAPPING
IGVII_3111 = BODY_AND_NOUN_BEARER_HOLD
```

Official IG bibliography identifies `IG VII` as Dittenberger’s 1892 *Inscriptiones Megaridis et Boeotiae* and includes a Lebadeia section. The old volume is public-domain and digitally catalogued, but the exact 3111 page/text has not yet been successfully extracted in the current runtime.

Searches for `3111` produce dangerous false-neighbor hits:

- `DVC 3111` is a **Dodona** private oracle inquiry of Nikomachos;
- `IG II² 3111` is an **Attic** inscription;
- neither is `IG VII 3111` from Lebadeia.

```text
DVC_3111_DODONA != IGVII_3111_LEBADEIA
IGII2_3111_ATTICA != IGVII_3111_LEBADEIA
IGVII_3111_PROPHETIS_NOUN_BEARER = HOLD
DO_NOT_COUNT_AS_NAMED_FEMALE_PERSON_YET
```

This is an identity firewall, not evidence against the Lebadeia reference itself.

---

# 6. Female cult office / visual-honour controls at Didyma

Public image honours prove that female cult personnel could be represented/honoured visually; they do not give a surviving identified prophetess portrait.

Examples:

- Nannion, Didyma 445 / I.Didyma 346, 7/6 BCE: hydrophoros, honoured `εἰκόνι γραπτῇ`;
- Didyma 478, AD 125/150: hydrophoros Apollonia, `εἰκόνι γραπτῇ`;
- other hydrophoros image/gilding controls in Didyma 439/444/511.

```text
DIDYMA_FEMALE_CULT_PUBLIC_IMAGE_HONOURS = A_B_HIGH
SURVIVING_IDENTIFIED_TRYPHOSA_EIRENE_PORTRAIT = NOT_FOUND
VISUAL_HONOUR != HEAD_CODE
```

---

# 7. Direct Didyma head/hair object — real, but wrong person/period

British Museum `1872,0608.1`, ca.540 BCE:
- female marble head from Didyma;
- veil covers back of head / much of hair;
- not identified as prophetess;
- centuries too early for Paul.

Official:
- https://www.britishmuseum.org/collection/object/G_1872-0608-1

```text
DIDYMA_ARCHAIC_VEILED_FEMALE_OBJECT = DIRECT_MUSEUM
IDENTIFIED_AS_PROPHETESS = FALSE
NEAR_PAULINE = FALSE
USE_FOR_TRYPHOSA_HEAD_CODE = PROHIBITED
```

---

# 8. Andania / Lycosura — direct ritual head/hair regulation

Detailed owner:
`00ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ_MASSEY_2015_ANDANIA_LYCOSURA_EPIGRAPHY_FULLTEXT_AUDIT_2026-08-10.md`

```text
ANDANIA_HIERAI_WHITE_PILOS = A_EPIGRAPHIC
ANDANIA_WOMEN_HAIR_ADORNMENT_RULES = A_EPIGRAPHIC
ANDANIA_HEADWEAR_AND_HAIR = DISTINCT_REGULATORY_AXES
ANDANIA_DATE = DISPUTED_91BCE_VS_C23CE
LYCOSURA_HAIR_AND_COVERING_DISTINCT_RULE_ITEMS = A_PRIMARY
LYCOSURA_EXACT_GENDER_MAPPING = OPEN
```

This proves that ritual headwear and hair can be separately regulated. It does **not** identify a prophetess’s head-state.

---

# 9. Literary / ritual-role comparison

```text
VIRGIL_AENEID6 = PROPHETIC_HAIR_TRANSFORMATION / A_PRIMARY_LITERARY
LUCAN_BC5 = FILLET_HEAD_DRESS_LAUREL_HAIR / A_PRIMARY_LITERARY
DIODORUS_16_26_6 = ROLE_SPECIFIC_PYTHIAN_COSTUME
IAMBLICHUS_DM_3_11 = DIDYMA_RITUAL_PREPARATION_WITHOUT_DIRECT_HEAD_CODE
DELPHI_PYTHIA_ROLE = WELL_ATTESTED
DELPHI_DIRECT_HISTORICAL_HEAD_HAIR_RULE = NOT_FOUND
```

Literary imagery/costume logic can show that prophetic states are visually markable; it cannot fill the missing epigraphic head-code.

---

# 10. Current Didyma edition / acquisition control

Wolfgang Günther, *Inschriften von Didyma: Supplement*, Didyma III.7 (2023), DOI `10.34780/a81b-ccaa`.

Official DAI:
- https://publications.dainst.org/books/dai/catalog/book/2102

DAI exposes a full-volume PDF route and the TOC/metadata show dedicated `Propheteninschriften` and `Hydrophoreninschriften` sections, but the binary endpoint still cache-fails in the current runtime.

```text
DIDYMA_SUPPLEMENT_2023 = CURRENT_OFFICIAL_EPIGRAPHIC_CONTROL
DIDYMA_SUPPLEMENT_PDF_ROUTE = VERIFIED_OFFICIAL
FULL_BODY_AUTOPSY = NOT_COMPLETED_RUNTIME_BINARY_HOLD
METADATA_OR_TOC != INSCRIPTION_BODY_READ
```

Nawotka publication-date firewall:

```text
CHAPTER_DOI = 10.13173/9783447125116.111
OA_FILE_COPYRIGHT_YEAR = 2025
CURRENT_HARRASSOWITZ_PRINT_DATE = 2026_02_11
PUBLICATION_YEAR = TYPE_SPECIFIC
```

---

# 11. Semantic / visual false-positive firewall

```text
KEKOSMEMENE != INVENTED_HEAD_ADORNMENT
MATRONA_STOLATA != HEAD_COVERING
PROPHETISKE_GRAFFITO != OFFICIAL_PROPHETIC_OFFICE
AGATHO_WIFE_OF_PROPHET != AGATHO_PROPHETIS
AMMIAS_THYATEIRA != AMMIA_PHILADELPHIA
THYATEIRA_ORACLE_FUNCTION != PROPHETIS_TITLE
IGLSYR_1_51_PROPHETIS != FEMALE_PERSON
IGII2_13148_REFERENCE != VERIFIED_PROPHETESS
DVC_3111 != IGVII_3111
IGII2_3111 != IGVII_3111
MALAY_NO137_SEQUENTIAL_POSITION != SEG49_1624_PROOF
VISUAL_HONOUR != SURVIVING_PORTRAIT
ARCHAIC_VEILED_WOMAN != FIRST_CENTURY_PROPHETESS
ADJACENT_NISYRA_STELE != TARGET_NISYRA_STELE
ADJACENT_MAIONIA_VEILED_FIGURE != POTTA
```

---

# 12. Bounded search result

Search families include:

```text
προφῆτις / προφῆτιδος / προφήτισα
πρόμαντις / προφητίσκη
+
ἀνάδημα / μίτρα / πῖλος / στέφανος
κόμη / τρίχες / κεκαλυμ-
veil / fillet / headband / hair / portrait / stele / relief
```

across PHI / SEG / TAM / IG / IK, Didyma/Miletos, wider Asia Minor, museum/object catalogues and specialist scholarship.

```text
SAME_IDENTIFIED_FEMALE_MANTIC_PERSON
+
SAME_OBJECT_SPECIFIC_VEIL_HEADBAND_HAIR_MARKER
=
NOT_FOUND_IN_BOUNDED_SEARCH

TRYPHOSA_HEAD_HAIR_CODE = NOT_ATTESTED
EIRENE_HEAD_HAIR_CODE = NOT_ATTESTED
POTTA_HEAD_HAIR_CODE = NOT_ATTESTED
AMMIAS_HEAD_HAIR_CODE = NOT_ATTESTED
NANAS_HEAD_HAIR_CODE = NOT_FOUND
APPHE_HEAD_HAIR_CODE = NOT_FOUND
ERESOS_PROPHETESS_OFFICE = A_EPIGRAPHIC_PRE_PAULINE_NO_HEAD_CODE
IGII2_13148 = VERY_LIKELY_REFERENCE_ERROR_NOT_PROPHETESS
NISYRA_1624 = OBJECT_PINNED_CONTEXT_NARROWED_NOUN_BEARER_HOLD
TERMESSOS_870 = CULT_CONTEXT_VERIFIED_NOUN_BEARER_HOLD
IGVII_3111 = IDENTITY_FIREWALLED_NOUN_BEARER_HOLD
GLOBAL_NONEXISTENCE = NOT_CLAIMED
```

---

# 13. Current implication for 1 Cor 11:4–5

No core grade reversal.

```text
MATERIAL_COVERING = B_HIGH_LEADING
HAIR_ONLY = C_SERIOUS_ALTERNATIVE
EXACT_CORINTH_TRIGGER = RECONSTRUCTION_LAYER

FEMALE_DIVINE_MEDIATION_IN_ASIA_MINOR = A_B_HIGH
PRE_PAULINE_GREEK_PROPHETESS_OFFICE_IN_RITUAL_SPACE = A_EPIGRAPHIC_CONTROL
EARLY_CHRISTIAN_FEMALE_PROPHECY_AFTER_PAUL = STRONG_RECEPTION_CONTROL
RITUAL_HEADWEAR_AND_HAIR_CAN_BE_SEPARATELY_REGULATED = A_PRIMARY
RITUAL_ROLE_CAN_CARRY_VISUAL_OR_COSTUME_CODE = B_HIGH_COMPARATIVE
DIRECT_NEAR_PAULINE_PROPHETESS_HEAD_HAIR_CODE = HOLD_NOT_VERIFIED
```

Safe synthesis:

> Female mantic/prophetic agency is directly attested in several Greek, Asia-Minor and early-Christian environments, and Eresos supplies a direct pre-Pauline sanctuary regulation in which a `prophetis` is an institutional exception to the general female temple-entry ban. The broadened corpus also demonstrates why reference lists and lexical hits cannot be counted mechanically: `IGLSyr 1 51` personifies an inscription as `prophetis`, while `IG II² 13148` converges across direct funerary text and official corpus organization as a very likely miscitation in the prophetess footnote. The remaining Nisyra, Termessos and Lebadeia entries have now been narrowed by independent object/cult/identity controls, but their `prophetis` noun-bearers remain unverified until direct bodies are acquired. No near-Pauline direct object has yet been verified in which the same identified prophetess/promantis bears a specific veil/headband/hair code capable of explaining Corinth.

---

# 14. Next-search queue

### P0 — object/body closure

1. Potta `TAM V.1 535 / PH263959` direct body.
2. Nanas **Tabbernee fig.77** / same-object monument image; text body is now closed.
3. Apphe `IK Kalchedon 61 = CIG 3796` direct page/object; CIG volume identity is closed.
4. Nisyra `SEG 49.1624 / PH348429 / TM949255` complete text and noun-bearer; explicitly prove/disprove Malay no.137 crosswalk rather than infer it.
5. Termessos `TAM III,1 870 / PH280975` direct text, noun-bearer and object type; Demeter Eleusinia context is already verified.
6. Lebadeia `IG VII 3111 / PH146626` direct body + noun-bearer; preserve `DVC 3111` / `IG II² 3111` identity firewall.
7. Didyma III.7 full body search for female mantic titles + head/hair terms.
8. Didyma 235B direct stone/Rehm page.

### P1

9. Find explicit erratum/intended object for Nawotka’s `IG II² 13148`; **do not guess**.
10. Corrington 1991 direct full text.
11. Claros personnel system with gender/status/object controls.
12. Additional historical images/bases tied to named mantic women.

---

# 15. Publication / provenance boundary

```text
PRODUCT_WRITE = false
SITE_PUBLICATION = false
PUBLICATION_HOLD = true
AGENT_DISCOVERY != AUTHORITY
DIRECT_OBJECT > SECONDARY_SUMMARY
REFERENCE_LIST != VERIFIED_PERSON_LIST
ROLE != HEAD_CODE
PRIESTESS != PROPHETIS_AUTOMATICALLY
LITERARY_TROPE != ETHNOGRAPHIC_FACT
VISUAL_CULTURE != IDENTIFIED_PROPHETESS_PORTRAIT
PHI_ORDINAL != EDITION_NUMBER_AUTOMATICALLY
OA_FILE_YEAR != PRINT_PUBLICATION_DATE_AUTOMATICALLY
NEGATIVE_BOUNDED_SEARCH != GLOBAL_NONEXISTENCE
