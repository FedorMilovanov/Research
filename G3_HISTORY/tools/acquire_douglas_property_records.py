...
UA = "FedorMilovanov-Research-G3-Property-Acquisition/1.1 (research-only; read-only public records)"
...
def fetch(op,url: str,timeout: int=90):
    req=urllib.request.Request(url,headers={"User-Agent":UA,"Accept":"text/html,application/xhtml+xml,*/*"}); started=time.time()
    try:
        with op.open(req,timeout=timeout) as r:
            b=r.read(); return b,{"requested_url":url,"final_url":r.geturl(),"status":getattr(r,"status",None),"content_type":r.headers.get("Content-Type"),"bytes":len(b),"sha256":sha(b),"elapsed_seconds":round(time.time()-started,3)}
    except urllib.error.HTTPError as e:
        body=e.read(); return body,{"requested_url":url,"final_url":e.geturl(),"status":e.code,"reason":str(e.reason),"content_type":e.headers.get("Content-Type"),"bytes":len(body),"sha256":sha(body),"elapsed_seconds":round(time.time()-started,3)}
    except Exception as e:
        return b"",{"requested_url":url,"status":None,"error":repr(e),"bytes":0,"elapsed_seconds":round(time.time()-started,3)}
...
def qpublic_checks(op,out: Path):
    ...
    for i,q in enumerate(QUERIES,1):
        ...
        results.append({"kind":"query","query":q,"fetch":m,"query_present":q.upper() in txt.upper(),"g3_present":"G3 MINISTRIES" in txt.upper(),"prays_mill_present":"PRAYS MILL" in txt.upper(),"parcel_present":"0038025021" in re.sub(r"\D","",txt),"text_preview":txt[:1000]})
    return results
...
def gsccca_checks(op,out: Path):
    ...
    return {"fetch":meta,"forms_found":len(parser.forms),"form_schema":parser.forms,"mentions_real_estate_index":"Real Estate" in txt,"mentions_search_name":"Search Name" in txt,"mentions_login":"Login" in txt,"note":"No authenticated or paid search is attempted. Form schema is preserved for a later fail-closed public-query patch if the unauthenticated endpoint permits it."}
...
def main():
    ...
    op=opener(); summary={"state":"EPHEMERAL_ACTION_ARTIFACT","publication_eligible":False,"queries":QUERIES,"authority":{},"qpublic":[],"gsccca":{},"errors":[]}
    ...
    summary["closure_boundary"]={"property_identity_closed":False,"deed_chain_closed":False,"counterparty_closed":False,"receivable_terms_closed":False,"warning":"Transport/access results are not negative evidence. No property is identified as the FY2022 $590k donation without official assessor/deed evidence."}
    writej(out/"SUMMARY.json",summary)
    print("=== G3 DOUGLAS PROPERTY PUBLIC-SURFACE DIAGNOSTIC ===")
