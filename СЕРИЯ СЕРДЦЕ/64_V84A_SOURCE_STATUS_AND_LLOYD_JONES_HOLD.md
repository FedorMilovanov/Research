# V84A — SOURCE STATUS / LIVE-URL VERIFICATION / LLOYD-JONES HOLD

**Дата:** 2026-07-29  
**Связь:** corrective addendum к `63_V84_DEPRESSION_SIN_SUFFERING_GUILT_BURNOUT_DESPAIR.md`  
**Причина:** сохранить абсолютную честность evidence-status после отказа live web-search/readback инструмента в текущем проходе.

---

## 1. Что V84 уже подтверждает независимо от новых URL

Следующие части имеют собственную authority и не зависят от вновь добавленных ссылок:

- библейская карта Христа, Иова, Павла, Пс. 88, Пс. 32/38/51, Илии, Ионы и 2 Кор. 7;
- V71 primary-source layer: Goodwin, Bridge, Sibbes, Rogers, Baxter, Gurnall, Guthrie;
- body-soul, medication and crisis gates V82–V83;
- ранее проверенные official Christian URLs из CCEF/BCC/ACBC/IBCD/Ligonier/RTS/PRTS/TMS;
- ранее проверенные NICE NG222, QS8, NG215 и RCPsych medication-safety URLs.

---

## 2. Newly introduced official URLs: current status

Эти URL указывают на широко известные официальные институциональные ресурсы, но в данном техническом сеансе live web tool не выполнил readback. До следующего успешного live-прохода их статус:

`OFFICIAL-URL / LIVE-READBACK-HOLD`

1. CCEF depression tag — `https://www.ccef.org/tag/depression/`
2. WHO depression fact sheet — `https://www.who.int/news-room/fact-sheets/detail/depression`
3. WHO burnout classification note — `https://www.who.int/news/item/28-05-2019-burn-out-an-occupational-phenomenon-international-classification-of-diseases`
4. WHO ICD-11 browser — `https://icd.who.int/browse/2025-01/mms/en`
5. NIMH depression overview — `https://www.nimh.nih.gov/health/topics/depression`
6. NICE CG91 — `https://www.nice.org.uk/guidance/cg91`
7. NICE NG134 — `https://www.nice.org.uk/guidance/ng134`
8. NICE CG192 — `https://www.nice.org.uk/guidance/cg192`
9. APA DSM information — `https://www.psychiatry.org/psychiatrists/practice/dsm`
10. APA depression guideline — `https://www.apa.org/depression-guideline`
11. VA/DoD MDD guideline — `https://www.healthquality.va.gov/guidelines/MH/mdd/`

### Rule

- Они могут оставаться в research ledger как `next verification queue`.
- Они не получают статус `M1 READ` до live readback.
- Ни одна точная цитата, дата обновления, диагностический критерий или процент не публикуется по ним до readback.
- Общая богословская позиция V84 не строится на них.

---

## 3. 58 links does not mean 58 equal evidence units

Правильная раскладка:

- ранее прочитанные full official texts;
- official abstracts/indexes;
- ранее проверенные official guidelines;
- новые official URL-HOLD;
- один отдельный book fulltext HOLD по Lloyd-Jones.

Количество ссылок никогда не заменяет evidence quality.

---

## 4. Martyn Lloyd-Jones

### Current honest status

- книга `Spiritual Depression: Its Causes and Cure` не найдена в Library;
- книга не найдена в Google Drive;
- новый полный текст не получен;
- страницы и цитаты не верифицированы;
- chapter map V84 — это reading agenda, а не утверждение о завершённом page-by-page чтении.

### What is allowed now

- указывать книгу как обязательный будущий primary source;
- объяснять, почему её дифференциальный подход важен;
- сохранять chapter questions;
- сопоставлять будущие находки с V71/V84 grids.

### What is forbidden now

- «книга прочитана полностью»;
- точные слова Ллойд-Джонса по памяти;
- страницы по вторичным цитатникам;
- вывод `Lloyd-Jones: all depression is sin`;
- вывод `Lloyd-Jones: bodily factors do not matter`.

---

## 5. Minor textual correction

В заголовке V84 `ПУРИ ТАНСКАЯ` следует читать как `ПУРИТАНСКАЯ`.

Исправление орфографическое и не меняет содержание.

---

## 6. Required next pass

1. Live readback 11 URL-HOLD.
2. Найти легальный full-text/экземпляр Lloyd-Jones.
3. Chapter-by-chapter ledger с page locators.
4. Отдельно проверить scientific literature по:
   - excessive/inappropriate guilt;
   - rumination and depression;
   - shame versus guilt;
   - depression following moral injury;
   - burnout versus depressive disorder;
   - grief, adjustment disorder and MDD differential.
5. Только после этого повышать V84 из `RESEARCH READY` в `ARTICLE READY`.
